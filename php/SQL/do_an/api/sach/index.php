...
if($_SERVER['REQUEST_METHOD'] == 'GET'){
    ...
    $id_insert = $xl_sach->them_sach($info_sach->ten_sach, $info_sach->id_tac_gia,
        $info_sach->gioi_thieu, $info_sach->doc_thu,$info_sach->id_loai_sach,
        $info_sach->id_nha_xuat_ban, $info_sach->so_trang,$info_sach->ngay_xuat_ban,
        $info_sach->kich_thuoc, $info_sach->sku,$info_sach->trong_luong,
        $info_sach->trang_thai, $info_sach->hinh,$info_sach->don_gia,
        $info_sach->gia_bia, $info_sach->noi_bat);
    echo json_encode(
        array(
            'error' => false,
            'message' => 'Post book sucessfuly',
            'data' => $id_insert
        )
    );
    //echo 'create data';
    ...
    echo json_encode(
        array(
            'error' => false,
            'message' => 'Delete book sucessfuly',
            'data' => $result
        )
    );
}
else {
    echo 'This method is not support.';
}
?>